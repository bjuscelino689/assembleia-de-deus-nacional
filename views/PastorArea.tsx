...
const PastorArea: React.FC<PastorAreaProps> = ({ 
  ...
  const syncMembersFromServer = async () => {
    try {
      ...
      setMembers(prev => {
        ...
        // Merge de Usuários Cadastrados (ignora deletados)
        if (Array.isArray(userList)) {
          userList.forEach(u => {
            if (u && u.id && u.name && u.id !== 'usr_admin_master' && !isUserOrMemberDeleted(u.id, u.email, u.phone, u.name)) {
              if (!map.has(u.id)) {
                map.set(u.id, {
                  id: u.id,
                  name: u.name,
  ...
  const handleConfirmDelete = async () => {
    ...
    if (type === 'member') {
      ...
      const targetPhone = targetMember?.phone;
      ...
      setMembers(prev => prev.filter(m => 
        ...
        (!targetPhone || m.phone !== targetPhone)
      ));
      ...
      // 6. Remove do Servidor Node.js em todas as rotas
      try {
      ...
      const cat = itemToDelete.categoryKey;
      if (cat === 'cultos') {
        setCults([]);
  ...
  const pendingMembers = members.filter(m => m && m.accessStatus === 'PENDENTE_LIBERACAO' && !m.isBlocked);
  const totalPending = pendingMembers.length;
  const totalBlocked = members.filter(m => m && (m.isBlocked || m.accessStatus === 'BLOQUEADO')).length;
  const totalActive = members.filter(m => m && m.accessStatus === 'LIBERADO' && !m.isBlocked).length;
  const totalOnline = members.filter(m => isMemberOnline(m)).length;
