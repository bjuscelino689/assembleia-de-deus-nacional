...
function permanentlyDeleteUserAccount(targetIdentifier?: string, additionalData?: { email?: string; phone?: string; name?: string }): { success: boolean; error?: string; members?: any[]; users?: any[] } {
  ...
  if (!cleanTarget && !extraEmail && !extraPhone) {
    return { success: false, error: "Identificador não fornecido." };
  }

  // NUNCA exclui o pastor ou administrador master
  if (
    cleanTarget === 'usr_admin_master' || 
    cleanTarget === 'm_pastor_master' || 
    cleanTarget === 'usr_pastor_master' || 
    cleanTarget === 'bjuscelino33@gmail.com' || 
    cleanTarget === 'meuplantaopro@gmail.com' ||
    extraEmail === 'bjuscelino33@gmail.com' ||
    extraEmail === 'meuplantaopro@gmail.com'
  ) {
    return { success: false, error: "O Administrador Master / Pastor Presidente não pode ser excluído." };
  }

  // Registra no Set de IDs excluídos todos os formatos
  if (cleanTarget) deletedUserIdentifiers.add(cleanTarget);
  if (cleanPhone && cleanPhone.length >= 8) deletedUserIdentifiers.add(cleanPhone);
  if (extraEmail) deletedUserIdentifiers.add(extraEmail);
  if (extraPhone && extraPhone.length >= 8) deletedUserIdentifiers.add(extraPhone);
  if (extraName && extraName.length > 2) deletedUserIdentifiers.add(extraName);

  // Busca e remove em serverMembers
  const matchedMembers = serverMembers.filter(m => {
    if (!m) return false;
    ...
    if (cleanTarget && (mId === cleanTarget || mEmail === cleanTarget || (cleanPhone.length >= 8 && mPhone === cleanPhone))) return true;
    if (extraEmail && mEmail === extraEmail) return true;
    if (extraPhone && extraPhone.length >= 8 && mPhone === extraPhone) return true;
    if (extraName && mName === extraName) return true;
    return false;
  });
  ...
  matchedMembers.forEach(m => {
    if (m.id) {
      ...
      deletedUserIdentifiers.add(mId);
      deleteDocumentFromFirestore("members", String(m.id)).catch(() => {});
      deleteDocumentFromFirestore("users", String(m.id)).catch(() => {});
    }
    if (m.email) {
      deletedUserIdentifiers.add(String(m.email).toLowerCase());
    }
    if (m.phone) {
      const pDigits = String(m.phone).replace(/\D/g, "");
      if (pDigits && pDigits.length >= 8) deletedUserIdentifiers.add(pDigits);
    }
  });

  // Busca e remove em serverRegisteredUsers
  const matchedUsers = serverRegisteredUsers.filter(u => {
    if (!u) return false;
    ...
    if (cleanTarget && (uId === cleanTarget || uEmail === cleanTarget || (cleanPhone.length >= 8 && uPhone === cleanPhone))) return true;
    if (extraEmail && uEmail === extraEmail) return true;
    if (extraPhone && extraPhone.length >= 8 && uPhone === extraPhone) return true;
    if (extraName && uName === extraName) return true;
    return false;
  });
  ...
  matchedUsers.forEach(u => {
    if (u.id) {
      ...
      deletedUserIdentifiers.add(uId);
      deleteDocumentFromFirestore("users", String(u.id)).catch(() => {});
      deleteDocumentFromFirestore("members", String(u.id)).catch(() => {});
    }
    if (u.email) {
      deletedUserIdentifiers.add(String(u.email).toLowerCase());
    }
    if (u.phone) {
      const pDigits = String(u.phone).replace(/\D/g, "");
      if (pDigits && pDigits.length >= 8) deletedUserIdentifiers.add(pDigits);
    }
  });

  // Deleta do Firestore com o identificador direto
  if (cleanTarget) {
  ...
  serverMembers = serverMembers.filter(m => {
    ...
    if (cleanTarget && (mId === cleanTarget || mEmail === cleanTarget || (cleanPhone.length >= 8 && mPhone === cleanPhone))) return false;
    if (extraEmail && mEmail === extraEmail) return false;
    if (extraPhone && extraPhone.length >= 8 && mPhone === extraPhone) return false;
    if (extraName && mName === extraName) return false;

    if (deletedUserIdentifiers.has(mId) || (mEmail && deletedUserIdentifiers.has(mEmail)) || (mPhone && deletedUserIdentifiers.has(mPhone))) return false;
    return true;
  });
  ...
  serverRegisteredUsers = serverRegisteredUsers.filter(u => {
    ...
    if (cleanTarget && (uId === cleanTarget || uEmail === cleanTarget || (cleanPhone.length >= 8 && uPhone === cleanPhone))) return false;
    if (extraEmail && uEmail === extraEmail) return false;
    if (extraPhone && extraPhone.length >= 8 && uPhone === extraPhone) return false;
    if (extraName && uName === extraName) return false;

    if (deletedUserIdentifiers.has(uId) || (uEmail && deletedUserIdentifiers.has(uEmail)) || (uPhone && deletedUserIdentifiers.has(uPhone))) return false;
    return true;
  });
  ...
  console.log(`[Exclusão Definitiva Concluída] Conta/Membro "${targetIdentifier || extraEmail || extraPhone}" foi removido definitivamente do servidor, Firestore e registros.`);
...
async function startServer() {
  ...
  app.delete("/api/users/:id", (req, res) => {
    const { id } = req.params;
    const result = permanentlyDeleteUserAccount(id);
    if (!result.success) {
