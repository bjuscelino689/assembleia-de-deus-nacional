...
import { saveVideoRecordLocally, StoredVideoRecord } from './videoStorage';
...
export async function uploadVideoWithProgress(
...
): Promise<{ videoId: string; playbackUrl: string; serverUrl?: string }> {
  onProgress(5);
  ...
  try {
    const directUrl = await uploadViaDirectFormData(fileOrBlob, fileName, (pct) => {
      onProgress(Math.min(95, 15 + Math.round(pct * 0.80)));
    });
    ...
    if (directUrl && directUrl.startsWith('/uploads/')) {
      ...
      await saveVideoRecordLocally(localRecord);
      onProgress(100);
      return { videoId, playbackUrl: directUrl, serverUrl: directUrl };
    }
  } catch (directErr) {
  ...
  try {
    const binaryUrl = await uploadViaBinaryStream(fileOrBlob, fileName, (pct) => {
      onProgress(Math.min(98, 20 + Math.round(pct * 0.78)));
    });
    ...
    if (binaryUrl && binaryUrl.startsWith('/uploads/')) {
      ...
      await saveVideoRecordLocally(localRecord);
      onProgress(100);
      return { videoId, playbackUrl: binaryUrl, serverUrl: binaryUrl };
    }
  } catch (binErr) {
  ...
  onProgress(100);
  return { videoId, playbackUrl: `idb://${videoId}` };
}
